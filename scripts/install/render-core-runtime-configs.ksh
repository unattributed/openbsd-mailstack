...
OUTPUT_ROOT="${PROJECT_ROOT}/services/generated/rootfs"
...
render_core() {
  ...
  render_template_file "${PROJECT_ROOT}/services/postfix/etc/postfix/main.cf.template" "${OUTPUT_ROOT}/etc/postfix/main.cf" \
    "MAIL_HOSTNAME=${MAIL_HOSTNAME}" "PRIMARY_DOMAIN=${PRIMARY_DOMAIN}" "POSTFIX_MYNETWORKS=${POSTFIX_MYNETWORKS}" \
    "POSTFIX_VMAIL_UID=${POSTFIX_VMAIL_UID}" "POSTFIX_VMAIL_GID=${POSTFIX_VMAIL_GID}" "POSTFIX_VMAIL_BASE=${POSTFIX_VMAIL_BASE}" \
    "TLS_CERT_PATH_FULLCHAIN=${TLS_CERT_PATH_FULLCHAIN}" "TLS_CERT_PATH_KEY=${TLS_CERT_PATH_KEY}" \
    "POSTFIX_RELAYHOST=${POSTFIX_RELAYHOST}" "RSPAMD_MILTER_PORT=${RSPAMD_MILTER_PORT}"
  render_template_file "${PROJECT_ROOT}/services/postfix/etc/postfix/master.cf.template" "${OUTPUT_ROOT}/etc/postfix/master.cf" \
    "POSTFIX_SUBMISSION_LOOPBACK_BIND=${POSTFIX_SUBMISSION_LOOPBACK_BIND}" "POSTFIX_SUBMISSION_VPN_BIND=${POSTFIX_SUBMISSION_VPN_BIND}" \
    "POSTFIX_SMTPS_LOOPBACK_BIND=${POSTFIX_SMTPS_LOOPBACK_BIND}" "POSTFIX_SMTPS_VPN_BIND=${POSTFIX_SMTPS_VPN_BIND}"
  for _tmpl in mysql_virtual_domains_maps.cf mysql_virtual_mailbox_maps.cf mysql_virtual_alias_maps.cf mysql_virtual_alias_domain_maps.cf; do
    render_template_file "${PROJECT_ROOT}/services/postfix/etc/postfix/${_tmpl}.template" "${OUTPUT_ROOT}/etc/postfix/${_tmpl}" \
      "POSTFIX_DB_NAME=${POSTFIX_DB_NAME}" "POSTFIX_DB_USER=${POSTFIX_DB_USER}" "POSTFIX_DB_PASSWORD=${POSTFIX_DB_PASSWORD}"
  done
  render_template_file "${PROJECT_ROOT}/services/postfix/etc/postfix/postscreen_access.cidr.template" "${OUTPUT_ROOT}/etc/postfix/postscreen_access.cidr" \
    "WIREGUARD_SUBNET=${WIREGUARD_SUBNET}" "LAN_SUBNET=${LAN_SUBNET}"
  cp "${PROJECT_ROOT}/services/postfix/etc/postfix/tls_policy.template" "${OUTPUT_ROOT}/etc/postfix/tls_policy"
  render_template_file "${PROJECT_ROOT}/services/postfix/etc/postfix/sasl_passwd.template" "${OUTPUT_ROOT}/etc/postfix/sasl_passwd" \
    "BREVO_SMTP_LOGIN=${BREVO_SMTP_LOGIN}" "BREVO_SMTP_PASSWORD=${BREVO_SMTP_PASSWORD}"

  render_template_file "${PROJECT_ROOT}/services/dovecot/etc/dovecot/dovecot.conf.template" "${OUTPUT_ROOT}/etc/dovecot/dovecot.conf" \
    "DOVECOT_LISTEN=${DOVECOT_LISTEN}"
  render_template_file "${PROJECT_ROOT}/services/dovecot/etc/dovecot/local.conf.template" "${OUTPUT_ROOT}/etc/dovecot/local.conf" \
    "DOVECOT_MAIL_LOCATION=${DOVECOT_MAIL_LOCATION}" "DOVECOT_VMAIL_UID=${DOVECOT_VMAIL_UID}" "DOVECOT_VMAIL_GID=${DOVECOT_VMAIL_GID}" \
    "DOVECOT_FIRST_VALID_UID=${DOVECOT_FIRST_VALID_UID}" "DOVECOT_LAST_VALID_UID=${DOVECOT_LAST_VALID_UID}" \
    "TLS_CERT_PATH_FULLCHAIN=${TLS_CERT_PATH_FULLCHAIN}" "TLS_CERT_PATH_KEY=${TLS_CERT_PATH_KEY}" \
    "DOVECOT_AUTH_MECHANISMS=${DOVECOT_AUTH_MECHANISMS}" "POSTFIX_VMAIL_BASE=${POSTFIX_VMAIL_BASE}" "POSTMASTER_EMAIL=${POSTMASTER_EMAIL}"
  render_template_file "${PROJECT_ROOT}/services/dovecot/etc/dovecot/dovecot-sql.conf.ext.template" "${OUTPUT_ROOT}/etc/dovecot/dovecot-sql.conf.ext" \
    "DOVECOT_DB_NAME=${DOVECOT_DB_NAME}" "DOVECOT_DB_USER=${DOVECOT_DB_USER}" "DOVECOT_DB_PASSWORD=${DOVECOT_DB_PASSWORD}"
  ...
  render_template_file "${PROJECT_ROOT}/services/nginx/etc/nginx/templates/rspamd.tmpl.template" "${OUTPUT_ROOT}/etc/nginx/templates/rspamd.tmpl" \
    "RSPAMD_CONTROLLER_BIND=${RSPAMD_CONTROLLER_BIND}" "WIREGUARD_SUBNET=${WIREGUARD_SUBNET}"
  render_template_file "${PROJECT_ROOT}/services/nginx/etc/nginx/templates/ssl.tmpl.template" "${OUTPUT_ROOT}/etc/nginx/templates/ssl.tmpl" \
    "TLS_CERT_PATH_FULLCHAIN=${TLS_CERT_PATH_FULLCHAIN}" "TLS_CERT_PATH_KEY=${TLS_CERT_PATH_KEY}"
  ...
  render_template_file "${PROJECT_ROOT}/services/nginx/etc/nginx/sites-available/main-ssl.conf.template" "${OUTPUT_ROOT}/etc/nginx/sites-available/main-ssl.conf" \
    "NGINX_HTTPS_BIND_1=${NGINX_HTTPS_BIND_1}" "NGINX_HTTPS_BIND_2=${NGINX_HTTPS_BIND_2}" "MAIL_HOSTNAME=${MAIL_HOSTNAME}" "WIREGUARD_BIND_IPV4=${WIREGUARD_BIND_IPV4}"
  render_template_file "${PROJECT_ROOT}/services/nginx/etc/nginx/control-plane.allow.template" "${OUTPUT_ROOT}/etc/nginx/control-plane.allow" \
  ...
  render_template_file "${PROJECT_ROOT}/services/postfixadmin/var/www/postfixadmin/config.local.php.template" "${OUTPUT_ROOT}/var/www/postfixadmin/config.local.php" \
    "POSTFIXADMIN_DB_USER=${POSTFIXADMIN_DB_USER}" "POSTFIXADMIN_DB_PASSWORD=${POSTFIXADMIN_DB_PASSWORD}" "POSTFIXADMIN_DB_NAME=${POSTFIXADMIN_DB_NAME}" \
    "POSTFIXADMIN_SETUP_PASSWORD_HASH=${POSTFIXADMIN_SETUP_PASSWORD_HASH}" "ABUSE_EMAIL=${ABUSE_EMAIL}" "HOSTMASTER_EMAIL=${HOSTMASTER_EMAIL}" \
    "POSTMASTER_EMAIL=${POSTMASTER_EMAIL}" "WEBMASTER_EMAIL=${WEBMASTER_EMAIL}" "DOMAIN_ADMIN_EMAIL=${DOMAIN_ADMIN_EMAIL}" \
  ...
  render_template_file "${PROJECT_ROOT}/services/roundcube/var/www/roundcubemail/config/config.inc.php.template" "${OUTPUT_ROOT}/var/www/roundcubemail/config/config.inc.php" \
    "ROUNDCUBE_DB_USER=${ROUNDCUBE_DB_USER}" "ROUNDCUBE_DB_PASSWORD=${ROUNDCUBE_DB_PASSWORD}" "ROUNDCUBE_DB_NAME=${ROUNDCUBE_DB_NAME}" \
    "MAIL_HOSTNAME=${MAIL_HOSTNAME}" "PRIMARY_DOMAIN=${PRIMARY_DOMAIN}" "ADMIN_EMAIL=${ADMIN_EMAIL}" "ROUNDCUBE_DES_KEY=${ROUNDCUBE_DES_KEY}"
  copy_tree_contents "${PROJECT_ROOT}/services/roundcube/etc/roundcube" "${OUTPUT_ROOT}/etc/roundcube"

  render_template_file "${PROJECT_ROOT}/services/rspamd/etc/rspamd/local.d/worker-controller.inc.template" "${OUTPUT_ROOT}/etc/rspamd/local.d/worker-controller.inc" \
    "RSPAMD_CONTROLLER_BIND=${RSPAMD_CONTROLLER_BIND}" "RSPAMD_CONTROLLER_SECURE_IP=${RSPAMD_CONTROLLER_SECURE_IP}" "RSPAMD_CONTROLLER_PASSWORD_HASH=${RSPAMD_CONTROLLER_PASSWORD_HASH}"
  render_template_file "${PROJECT_ROOT}/services/rspamd/etc/rspamd/local.d/worker-proxy.inc.template" "${OUTPUT_ROOT}/etc/rspamd/local.d/worker-proxy.inc" \
    "RSPAMD_MILTER_BIND=${RSPAMD_MILTER_BIND}" "RSPAMD_NORMAL_BIND=${RSPAMD_NORMAL_BIND}"
  render_template_file "${PROJECT_ROOT}/services/rspamd/etc/rspamd/local.d/redis.conf.template" "${OUTPUT_ROOT}/etc/rspamd/local.d/redis.conf" \
    "RSPAMD_REDIS_HOST=${RSPAMD_REDIS_HOST}" "RSPAMD_REDIS_PORT=${RSPAMD_REDIS_PORT}"
  render_template_file "${PROJECT_ROOT}/services/rspamd/etc/rspamd/local.d/antivirus.conf.template" "${OUTPUT_ROOT}/etc/rspamd/local.d/antivirus.conf" \
    "RSPAMD_CLAMAV_SOCKET=${RSPAMD_CLAMAV_SOCKET}"
  render_template_file "${PROJECT_ROOT}/services/rspamd/etc/rspamd/local.d/dkim_signing.conf.template" "${OUTPUT_ROOT}/etc/rspamd/local.d/dkim_signing.conf" \
    "DKIM_SELECTOR=${DKIM_SELECTOR}" "WIREGUARD_SUBNET=${WIREGUARD_SUBNET}"
  ...
  ensure_directory "${PROJECT_ROOT}/services/generated"
  cat > "${PROJECT_ROOT}/services/generated/summary.txt" <<EOS
Rendered core runtime rootfs
...
EOS
  log_info "rendered core runtime config set into ${OUTPUT_ROOT}"
}
