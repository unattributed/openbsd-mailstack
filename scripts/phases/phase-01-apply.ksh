...
collect_inputs() {
  ...
  prompt_value "WAN_INTERFACE" "Enter the WAN interface or macro" "${WAN_INTERFACE}"
  prompt_value "LAN_IPV4" "Enter the LAN IPv4 address" "${LAN_IPV4}"
  prompt_value "LAN_CIDR" "Enter the LAN prefix length" "${LAN_CIDR}"
  ...
  if [ "${ENABLE_WIREGUARD}" = "yes" ]; then
    ...
    prompt_value "WIREGUARD_SERVER_IPV4" "Enter the WireGuard server IPv4" "${WIREGUARD_SERVER_IPV4}"
