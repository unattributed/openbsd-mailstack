...
save_configs_if_requested() {
  ...
  write_named_config "${DOMAINS_CONF}"     "PRIMARY_DOMAIN" "${PRIMARY_DOMAIN}"     "DOMAINS" "${DOMAINS}"     "INITIAL_MAILBOXES" "${INITIAL_MAILBOXES}"     "DOMAIN_ADMIN_EMAIL" "${DOMAIN_ADMIN_EMAIL}"     "POSTFIX_VIRTUAL_TRANSPORT" "${POSTFIX_VIRTUAL_TRANSPORT:-dovecot}"     "DOVECOT_MAIL_LOCATION" "${DOVECOT_MAIL_LOCATION}"     "VMAIL_UID" "${VMAIL_UID}"     "VMAIL_GID" "${VMAIL_GID}"
